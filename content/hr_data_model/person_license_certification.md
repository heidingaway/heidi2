---
uri: https://gcxgce.sharepoint.com/teams/10001579/#person_license_certification
title: Person License-Certification
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#area_person
- https://gcxgce.sharepoint.com/teams/10001579/#person_license_certification
- https://schema.org/Person
draft: false
---

## Related Links

- [[area_person]]
- [[person]]

## Semantic Connections

```mermaid
graph TD
  Person_License_Certification["Person License-Certification"]:::current-page-node
  area_person["area_person"]
  person["person"]
  Person_License_Certification-->|" subClassOf "|area_person
  person-->|" licensed "|Person_License_Certification
```
