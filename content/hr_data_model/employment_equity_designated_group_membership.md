---
uri: https://gcxgce.sharepoint.com/teams/10001579/#employment_equity_designated_group_membership
title: Employment Equity Designated Group Membership
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#area_person
- https://gcxgce.sharepoint.com/teams/10001579/#employment_equity_designated_group_membership
- https://gcxgce.sharepoint.com/teams/10001579/#person
draft: false
---

## Related Links

- [[area_person]]
- [[employment_equity_designated_group_membership]]
- [[person]]

## Semantic Connections

```mermaid
graph TD
  Employment_Equity_Designated_Group_Membership["Employment Equity Designated Group Membership"]:::current-page-node
  area_person["area_person"]
  person["person"]
  person-->|" memberOf "|Employment_Equity_Designated_Group_Membership
  Employment_Equity_Designated_Group_Membership-->|" subClassOf "|area_person
```
