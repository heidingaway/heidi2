---
uri: https://gcxgce.sharepoint.com/teams/10001579/#advertising_campaign
title: Advertising Campaign
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#advertising_campaign
- https://gcxgce.sharepoint.com/teams/10001579/#area_staffing
draft: false
---

## Related Links

- [[advertising_campaign]]
- [[area_staffing]]

## Semantic Connections

```mermaid
graph TD
  Advertising_Campaign["Advertising Campaign"]:::current-page-node
  area_staffing["area_staffing"]
  Advertising_Campaign-->|" subClassOf "|area_staffing
```
