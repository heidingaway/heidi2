---
uri: https://gcxgce.sharepoint.com/teams/10001579/#competency_type
title: competency_type
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#area_competency
- https://gcxgce.sharepoint.com/teams/10001579/#competency_dictionary
- https://gcxgce.sharepoint.com/teams/10001579/#competency_type
draft: false
---

## Related Links

- [[area_competency]]
- [[competency_dictionary]]
- [[competency_type]]

## Semantic Connections

```mermaid
graph TD
  competency_type["competency_type"]:::current-page-node
  area_competency["area_competency"]
  competency_dictionary["competency_dictionary"]
  competency_type-->|" subClassOf "|area_competency
  competency_type-->|" includedIn "|competency_dictionary
```
