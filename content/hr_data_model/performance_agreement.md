---
uri: https://gcxgce.sharepoint.com/teams/10001579/#performance_agreement
title: Performance Agreement
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#area_performance
- https://gcxgce.sharepoint.com/teams/10001579/#performance_agreement
draft: false
---

## Related Links

- [[area_performance]]
- [[performance_agreement]]

## Semantic Connections

```mermaid
graph TD
  Performance_Agreement["Performance Agreement"]:::current-page-node
  area_performance["area_performance"]
  Performance_Agreement-->|" subClassOf "|area_performance
```
