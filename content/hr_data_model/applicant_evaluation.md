---
uri: https://gcxgce.sharepoint.com/teams/10001579/#applicant_evaluation
title: Applicant Evaluation
mermaid_layers: 1
entities:
- https://gcxgce.sharepoint.com/teams/10001579/#applicant_evaluation
- https://gcxgce.sharepoint.com/teams/10001579/#area_staffing
draft: false
---

## Related Links

- [[applicant_evaluation]]
- [[area_staffing]]

## Semantic Connections

```mermaid
graph TD
  Applicant_Evaluation["Applicant Evaluation"]:::current-page-node
  area_staffing["area_staffing"]
  Applicant_Evaluation-->|" subClassOf "|area_staffing
```
